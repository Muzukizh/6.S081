...
// add for Lab3
void ukvmmap(pagetable_t kernelPt, uint64 va, uint64 pa, uint64 sz, int perm)
{
...
void vmprint_basis(pagetable_t pagetable, int level)
{
  for (int i = 0; i < 512; i++)
  {
    pte_t pte = pagetable[i];
    if (pte & PTE_V)
    {
      ...
      {
        vmprint_basis((pagetable_t)pa, level + 1);
      }
    }
  }
}
